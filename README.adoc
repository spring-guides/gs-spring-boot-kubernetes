...
NOTE: If you don't have `docker` locally or want to automatically push an image to a registry then Spring Boot Maven or Gradle plugins would be a good choice. They use https://buildpacks.io/[Cloud Native Buildpacks], and can easily be automated in an enterprise setting, when you need a trusted build service for a CI/CD pipeline.
